...
for dn in dirs:
    ...
    n += 1
    # geometry
    occ_err.append(abs(gp.values["occupancy"] - s["occupancy"]))
    cx_err.append(abs(gp.values["object_center_x"] - s["object_center_x"]))
    cy_err.append(abs(gp.values["object_center_y"] - s["object_center_y"]))
    shot_ok += (vocab._classify(gp.values["occupancy"], vocab.SHOT_SIZE) == vocab._classify(s["occupancy"], vocab.SHOT_SIZE))
    place_n += 1
    place_ok += (vocab._classify(gp.values["object_center_x"]/W, vocab.PLACE_X) == vocab._classify(s["object_center_x"]/W, vocab.PLACE_X))
    # bearing
...
print(f"  object_center: MAE=({np.mean(cx_err):.0f},{np.mean(cy_err):.0f})px   placement-x CATEGORY match={100*place_ok/place_n:.0f}%")
