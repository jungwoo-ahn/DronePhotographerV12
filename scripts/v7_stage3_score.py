...
Yaw re-render, and why this script needs NO change for it
---------------------------------------------------------
`v7_rerender_yaw.py` spins the subject by replacing the v6 placement's Z rotation
with the planned yaw: ``rotation = [rx, ry, radians(yaw_deg)]``. This script still
builds ``R_obj`` from the *v6* rotation, which looks wrong and is not: every v6
placement has rz == 0 (verified, 958/958), so R_obj is the identity about Z and
``cam_to_obj_azimuth_deg`` comes out in the WORLD frame. The re-render replays the
same camera trajectory, so that world azimuth is identical before and after — as it
should be, since rotating the subject does not move the camera.

The yaw therefore does not belong here. It enters downstream, where the subject's
facing is applied:

    effective_front_az = front_az[asset] + placement_yaw_deg
    bearing            = effective_front_az - azimuth

Scoring this data with a yaw-aware R_obj would double-count the rotation.

Usage on the re-render (out-dir has no assignment file, so it walks the directory):

    python3 scripts/v7_stage3_score.py --out-dir runs/rerender_yaw --resume
